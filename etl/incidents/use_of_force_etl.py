...
binary_prefixes = ['MEMRESP','SUBAC','SUBJECT']
...
def go():
    ...
    uof_df = recode_yesno(uof_df)
    # uof_beatyear = add_features_at_beatyear(uof_df)
    
    if not os.path.exists('../../data/features/incidents'):
        os.mkdir('../../data/features/incidents')
    
    uof_df.to_csv('../../data/features/incidents/use_of_force.csv', index=False)
